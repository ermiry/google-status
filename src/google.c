...
static void google_env_get_runtime (void) {
...
static unsigned int google_env_get_port (void) {
...
static void google_env_get_cerver_receive_buffer_size (void) {
...
static void google_env_get_cerver_th_threads (void) {
...
static void google_env_get_cerver_connection_queue (void) {
...
static unsigned int google_init_env (void) {

	unsigned int errors = 0;

	google_env_get_runtime ();

	errors |= google_env_get_port ();

	google_env_get_cerver_receive_buffer_size ();

	google_env_get_cerver_th_threads ();

	google_env_get_cerver_connection_queue ();
...
unsigned int google_init (void) {

	unsigned int errors = 0;

	if (!google_init_env ()) {
		errors |= google_service_init ();
	}
